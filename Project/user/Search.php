...
<div aria-live="polite" aria-atomic="true" class="position-fixed bottom-0 end-0 p-3" style="z-index: 11">
    ...
    <div class="filters" id="sidebar">
        ...
        ?>
            <div class="form-check">
                <input class="form-check-input" type="checkbox" value="<?php echo $data['type_id']; ?>" id="type<?php echo $data['type_id']; ?>" name="type" onchange="getcategory(this.value),getSearch()">
                <label class="form-check-label" for="type<?php echo $data['type_id']; ?>">
        ...
        ?>
            <div class="form-check">
                <input class="form-check-input" type="checkbox" value="<?php echo $data['size_id']; ?>" id="size<?php echo $data['size_id']; ?>" name="size" onchange="getSearch()">
                <label class="form-check-label" for="size<?php echo $data['size_id']; ?>">
                    <?php echo $data['size_name']; ?>
                </label>
            </div>
        <?php
        }
        ...
        ?>
            <div class="form-check">
                <input class="form-check-input" type="checkbox" value="<?php echo $data['color_id']; ?>" id="color<?php echo $data['color_id']; ?>" name="color" onchange="getSearch()">
                <label class="form-check-label" for="color<?php echo $data['color_id']; ?>">
        ...
        ?>
            <div class="form-check">
                <input class="form-check-input" type="checkbox" value="<?php echo $data['material_id']; ?>" id="material<?php echo $data['material_id']; ?>" name="material" onchange="getSearch()">
                <label class="form-check-label" for="material<?php echo $data['material_id']; ?>">
    ...
    <script>
        ...
        function getcategory(seltype) {
    $.ajax({
        url: "../Assets/AjaxPages/AjaxSearchCat.php?type=" + seltype,
        method: "GET",
        success: function (response) {
            document.getElementById("selcategory").innerHTML = response;
        }
    });
}
        function getSearch() {
            const category = getSelectedCheckboxes("category");
            const type = getSelectedCheckboxes("type");
            const material = getSelectedCheckboxes("material");
            const color = getSelectedCheckboxes("color");
            const size = getSelectedCheckboxes("size");
            const searchKeyword = document.getElementById("search-field").value;

            $.ajax({
                url: "../Assets/AjaxPages/AjaxSearchP.php?search="+searchKeyword+"&category="+category+"&type="+type+"&color="+color+"&size="+size+"&material="+material,
                method: "GET",
                success: function (response) {
